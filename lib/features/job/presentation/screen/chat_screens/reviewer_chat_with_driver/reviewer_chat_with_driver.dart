...
@RoutePage()
class ChatWithDriverScreen extends HookConsumerWidget {
  ...
  Stream<String> _getConversationStream(StaffChatManager staffChatManager) {
    return Stream.fromFuture(
      staffChatManager.getOrCreateStaffConversation(driverId, 'driver'),
    );
  }
}
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isSent = message.senderId == staffChatManager.currentUserId;

                  return ChatMessage(
                    text: message.content,
                    time: _formatTimestamp(message.timestamp),
                    isSent: isSent,
                    senderName: !isSent ? driverName : 'Bạn',
                    avatarUrl: !isSent ? driverAvatar : null,
                  );
                },
              ),
            ),
            ChatInputBox(
              onSendMessage: (content) async {
                if (content.trim().isNotEmpty) {
                  ...
                  await staffChatManager.sendStaffMessage(conversationId, content);
                }
              },
